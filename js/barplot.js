...
var x1 = d3.scaleLinear().range([0, width / 2 - lengthMiddle / 2]);
var x1Axis = svg_barplot
  .append("g")
  .attr("transform", "translate(0," + height + ")");

// Initialize the second X axis (Right)
var x2 = d3.scaleLinear().range([width / 2 + lengthMiddle / 2, width]);
var x2Axis = svg_barplot
  .append("g")
  .attr("transform", "translate(0," + height + ")");
...
// Reorder data
var data = dataBarplot.sort(function(a, b) {
  return (
...
var myPrior = d3
  .map(data, function(d) {
    return d.Prior_disorder;
  })
  .keys();
var myLater = d3
  .map(data, function(d) {
    return d.Later_disorder;
  })
  .keys();
...
function update(variable) {
  // get the data
  var don1 = data.filter(function(d) {
    return d.Prior_disorder == variable;
  });
  var don2 = data.filter(function(d) {
    return d.Later_disorder == variable;
  });

  // biggest HR?
  var a = d3.max(don1, function(d) {
    return +d.HR;
  });
  var b = d3.max(don2, function(d) {
    return +d.HR;
  });
  ...
  // Add bars graph left
  var u1 = svg_barplot.selectAll(".barleft").data(don1);
  ...
  u1.enter()
    ...
    .attr("opacity", 0.7)
    .style("fill", function(d) {
      return myColor(myGroup(d.Later_disorder));
    })
    // Go to right Y position
    .transition()
    .duration(0)
    .attr("y", function(d) {
      return y(d.Later_disorder);
    })
  ...
  var u2 = svg_barplot.selectAll(".barright").data(don2);
  ...
  u2.enter()
    ...
    .attr("opacity", 0.7)
    .style("fill", function(d) {
      return myColor(myGroup(d.Prior_disorder));
    })
    // Go to right Y position
    .transition()
    .duration(0)
    .attr("y", function(d) {
      return y(d.Prior_disorder);
    })
    .transition()
    .duration(1000)
    .attr("width", function(d) {
...
d3.select("#linkShowNicotine").on("click", function() {
  update("Nicotine dependence");
});

// Add a listener to the button for selection and run the update function if needed
d3.select("#dropdown-disorder").on("change", function(d) {
